...
exports.run = async (client, message, args) => {
    ...
    try {
        ...
        message.channel.send(new Discord.MessageEmbed().setDescription(`**__Kayıt İşlemi Başarılı__**\n\n<:KannaSip:783940980083916800> Kayıt Edilen Kişi: ${ada}\n<:KannaSip:783940980083916800> Kayıt Yapan Yetkili: ${message.author}\n<:KannaSip:783940980083916800> Kayıt İşleminde Verilen Rol: <@&${adiş.kizRol}>\n<:KannaSip:783940980083916800> Kayıt İşleminde Alınan Rol: <@&${adiş.kayitsiz}>`).setFooter(`Akeno - Kayıt Sistemi`).setColor("RANDOM").setTimestamp())
        ada.setNickname(`${damla} | ${damliş}`).catch(e => message.channel.send(`Benden Üstte Olduğu İçin İsmini Değiştiremedim.`))
        await ada.roles.add(adiş.kizRol) //eğer başka rolleriniz de varsa onları da ek olarak congif.json da belirtip alt satıra kopyalayıp yapın.
        await ada.roles.remove(adiş.kayitsiz)
        message.guild.channels.cache.get(adiş.genelChat).send(`${ada} aramıza katıldı :tada: Sunucumuz şuanda **${message.guild.memberCount}** kişi!`)
